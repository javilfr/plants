...
def run_agent(user_message: str, history: list) -> str:
    ...
    Builds the message list from history, then loops: call the LLM, execute any
    tool calls, append results, repeat — until the LLM replies with no tool calls
    ...
    for _ in range(MAX_TOOL_ROUNDS):
        ...
        # Append the assistant's turn (with tool_calls) before any tool results
    ...
    # MAX_TOOL_ROUNDS reached — do one final call without tools to get a text reply
    response = _client.chat.completions.create(
        model=LLM_MODEL,
        messages=messages,
    )
    return response.choices[0].message.content or "I reached the maximum number of tool calls without a final answer."
